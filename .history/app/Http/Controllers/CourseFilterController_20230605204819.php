...
class CourseFilterController extends Controller
{
    ...
    public function display(Request $request)
    {
        ...
        $courseCount = Course::where('name', 'LIKE', '%' . $request->search . '%')->count();
        ...
        foreach($attributes as $attribute) {
            $attributeItem = $attribute->attributeItems;
        ...
        $response['data_loaded_count'] = $courses->count();
        $response['data_total_count'] = $courseCount;
        
        return view('frontend-home', ['filterData' => $response]);
    }
    ...
    {
        return response()->json($this->getApiFilterData($request));
    }



    public function getApiFilterData(Request $request)
    {
        ...
        if ($request->has('server_side_update') && $request->server_side_update == 1) {
            ...
            if ($request->has('data_page_count')) {
                ...
                $dataCount = $dataCount === 'all' ? 99999 : $dataCount;
            ...
            if ($request->has('tags') && $request->tags[0]) {
                ...
                $filterCourseIdsTagsRq = CourseTag::whereIn('tag_id', $tagIds)->pluck('course_id')->toArray();
            }

            if ($request->has('categories') && $request->categories[0]) {
                $categoryIds = explode(',', $request->categories[0]);
                $filterCourseIdsCategoriesRq = CourseCategory::whereIn('category_id', $categoryIds)->pluck('course_id')->toArray();
            }

            if ($request->has('attr') && $request->attr[0]) {
                $attributes = explode(',', $request->attr[0]);
                $filterCourseIdsAttributeRq = CourseAttributeItem::whereIn('attribute_attribute_item_id', $attributes)->pluck('course_id')->toArray();
            }

            if (isset($filterCourseIdsCategoriesRq)) {
                $filterCourseIds = $filterCourseIdsCategoriesRq;
            }

            if (isset($filterCourseIdsTagsRq)) {
                $filterCourseIds = isset($filterCourseIdsCategoriesRq) ? array_intersect($filterCourseIds, $filterCourseIdsTagsRq) : $filterCourseIdsTagsRq;
            }

            if (isset($filterCourseIdsAttributeRq)) {
                $filterCourseIds = isset($filterCourseIdsCategoriesRq) || isset($filterCourseIdsTagsRq) ? array_intersect($filterCourseIds, $filterCourseIdsAttributeRq) : $filterCourseIdsAttributeRq;
            }

            if ($request->has('sort')) {
                $sortType = explode('#', $request->sort)[0];
                $sortType = $sortType === 'date' ? 'created_at' : $sortType;
                $sortOrder = explode('#', $request->sort)[1];
            }

            if (isset($request->search)) {
                if (isset($filterCourseIds)) {

                    if ($request->has('sort')) {
                        $courses = Course::with(['courseAttributeItems', 'author', 'categories'])
                            ->whereIn('id', $filterCourseIds)
                    ...
                    $courseCount = Course::whereIn('id', $filterCourseIds)
                        ->where('name', 'LIKE', '%' . $request->search . '%')
                        ->count();
                    ...
                    $courseCount = Course::where('name', 'LIKE', '%' . $request->search . '%')
                        ->count();
                }
            } else {
                if (isset($filterCourseIds)) {
            ...
            $courses = Course::with(['courseAttributeItems', 'author', 'categories'])->select(['id', 'name', 'excerpt', 'featured_image', 'reviews_allowed', 'rating_items', 'rating', 'created_at'])
            ->orderBy('created_at', 'desc')
            ->get();
