        ...
        if ($request->has('server_side_update') && $request->server_side_update == 1) {
            ...
            if (isset($request->search)) {
                if (isset($filterCourseIds)) {
                    ...
                    $courseCount = Course::with(['courseAttributeItems', 'author', 'categories'])
                        ->whereIn('id', $filterCourseIds)
                        ->where('name', 'LIKE', '%' . $request->search . '%')
                        ->count();
                ...
                if (isset($filterCourseIds)) {
                    if ($request->has('sort')) {
                        $courses = Course::with(['courseAttributeItems', 'author', 'categories'])->whereIn('id', $filterCourseIds)
                            ->orderBy($sortType, $sortOrder)
                            ->offset($dataOffset)
                            ->take($dataCount)
                            ->get();
                    ...
                    $courseCount = Course::with(['courseAttributeItems', 'author', 'categories'])->whereIn('id', $filterCourseIds)
                        ->offset($dataOffset)
                        ->take($dataCount)
                        ->count();
