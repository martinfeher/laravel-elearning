...
class CourseFilterController extends Controller
{
    ...
    public function getFilterData(Request $request)
    {
        ...
        if ($request->has('server_side_update')){
            ...
            // $courses = Course::with(['courseAttributeItems', 'author', 'categories'])->select(['id', 'name', 'excerpt', 'featured_image', 'reviews_allowed', 'rating_items', 'rating', 'created_at'])->offset($dataOffset)->take($dataCount)->get();
            
            if ($request->has('categories') && !empty($request->categories[0])){
                $categoryIds = explode(',', $request->categories[0]);

                $courseIds = CourseCategory::whereIn('category_id', $categoryIds)->pluck('course_id');
                $courses = Course::with(['courseAttributeItems', 'author', 'categories'])->whereIn('id', $courseIds)
                    ->offset($dataOffset)
                    ->take($dataCount)
                    ->get();
                // $courses = Course::with(['courseAttributeItems', 'author', 'categories'])->whereHas('categories', function($q) use ($categoryIds)
                // {
                ...
                $courses = Course::with(['categories', 'courseAttributeItems', 'author', 'categories'])
            ...
            $courses = Course::with(['categories', 'courseAttributeItems', 'author'])->select(['id', 'name', 'excerpt', 'featured_image', 'reviews_allowed', 'rating_items', 'rating', 'created_at'])->get();
