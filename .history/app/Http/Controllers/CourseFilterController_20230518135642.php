...
class CourseFilterController extends Controller
{
    ...
    public function getFilterData(Request $request)
    {
        ...
        if ($request->has('server_side_update')){
            ...
            // $courses = Course::with(['courseAttributeItems', 'author', 'categoriesItems'])->select(['id', 'name', 'excerpt', 'featured_image', 'reviews_allowed', 'rating_items', 'rating', 'created_at'])->offset($dataOffset)->take($dataCount)->get();
            
            if ($request->has('categories') && !empty($request->categories[0])){
                $categoryIds = explode(',', $request->categories[0]);

                $courses = Course::with(['courseAttributeItems', 'author'])->whereHas('categories', function($q) use ($categoryIds)
                // $courses = Course::with(['courseAttributeItems', 'author', 'categoriesItems'])->whereHas('categories', function($q) use ($categoryIds)
                {
                    $q->whereIn('id', [1, 5]);
