    ...
    public function getFilterData(Request $request)
    {
        ...
        if ($request->has('server_side_update')){
            ...
            if ($request->has('tags')) {
                ...
                $courseIdsTagsRq = CourseTag::whereIn('tag_id', $tagIds)->pluck('course_id');
            }

            if ($request->has('categories')) {
                $categoryIds = explode(',', $request->categories[0]);
                $courseIdsCategoriesRq = CourseCategory::whereIn('category_id', $categoryIds)->pluck('course_id');
            }

            if (!empty($courseIdsTagsRq) && !empty($courseIdsCategoriesRq)) {
                $courseIds = array_intersect($courseIdsTagsRq, $courseIdsCategoriesRq);
            } else if (!empty($courseIdsTagsRq)) {
