...
class CourseFilterController extends Controller
{
    ...
    public function getFilterData()
    {
        ...
        foreach($courses as $course) {
            ...
            dd($attributesMappedIdName);

            $courseAttributeItems = $course->courseAttributeItems;

            $attributeItemJson = [];
            foreach ($courseAttributeItems as $item) {
                $productProductAttributeKey = $attributeKey === 'name' ? $attributesMappedIdName[$item->attribute_id] : $item->attribute_id;
                $attributeItemValue = $attributeKey === 'name' ? $attributeItemsMappedIdName[$item->attribute_item_id] : $item->attribute_item_id;

                if (!isset($attributeItemJson[$productProductAttributeKey])) {
                    $attributeItemJson[$productProductAttributeKey] = [$attributeItemValue];
                } else {
                    array_push($attributeItemJson[$productProductAttributeKey], $attributeItemValue);
                }
            }
