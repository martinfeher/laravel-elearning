    ...
    public function getFilterData(Request $request)
    {
        ...
        if ($request->has('server_side_update') && $request->server_side_update == 1){
            ...
            if ($request->has('attr') && $request->attr[0]) {
                ...
                $attribute_id = explode('#', $attributes[0])[0];
                $attribute_item_id = explode('#', $attributes[0])[1];
                // dd($attributes);
                $courseIdsAttributeRq = CourseAttributeItem::where('attribute_attribute_item_id', $attribute_id)->where('attribute_item_id', $attribute_item_id)->pluck('course_id')->toArray();
                $courseIdsAttributeRq = CourseAttributeItem::where('attribute_id', $attribute_id)->where('attribute_item_id', $attribute_item_id)->pluck('course_id')->toArray();
                // dd($courseIdsAttributeRq);
            }

            // if (isset($courseIdsTagsRq) && isset($courseIdsCategoriesRq) && isset($courseIdsAttributeRq)) {
            //     $courseIds = array_intersect($courseIdsTagsRq, $courseIdsCategoriesRq);
