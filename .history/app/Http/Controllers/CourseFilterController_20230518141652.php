    ...
    public function getFilterData(Request $request)
    {
        ...
        if ($request->has('server_side_update')){
            ...
            if ($request->has('tags') && !empty($request->tags[0])){
                ...
                $courseIds = CourseTag::whereIn('tag_id', $tagIds)->pluck('course_id');
            }


            if ($request->has('categories') && !empty($request->categories[0])){
                $categoryIds = explode(',', $request->tags[0]);

                $courseIds = CourseCategory::whereIn('tag_id', $tagIds)->pluck('course_id');
            }
